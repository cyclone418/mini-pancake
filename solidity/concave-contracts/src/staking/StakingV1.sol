...
import {Position}                    from "./StakingStorageV1.sol";
import {Pool}                        from "./StakingStorageV1.sol";
...
contract StakingV1 is StakingStorageV1, Initializable, AccessControlUpgradeable, PausableUpgradeable, ERC721Upgradeable {
    ...
    function unlock(
        ...
        uint256 baseObligation = shares.mulDivDown(_poolIndex(pool.balance, pool.supply), 1e18);
        // Calculate excess amount obligated to user
        uint256 excessObligation = shares.mulDivDown(pool.rewardsPerShare, 1e18) - position.rewardDebt;
    ...
    function _poolIndex(
        ...
        return uint256(1e18).mulDivDown(_bal, _supply);
    }
    ...
    function viewPositionRewards(
        ...
        // Calculate base amount obligated to user
        baseRewards = shares.mulDivDown(_poolIndex(pool.balance, pool.supply), 1e18);
        // Calculate excess amount obligated to user
        excessRewards = shares.mulDivDown(pool.rewardsPerShare, 1e18) - position.rewardDebt;
    ...
    function viewStakingCap(uint256 poolNum) public view virtual returns(uint256) {
        ...
        uint256 lhs = 1e18 - coopRateMax - uint256(1e18 - coopRateMax).mulDivDown(1e18, minPrice);
        ...
        for (uint256 i; i < poolsLength;) {
            ...
            if (poolNum != i) {
                ...
                if (_balance != 0) lgm += _balance.mulDivDown(lp.g, 1e18);
            }
            unchecked { ++i; }
        }
    ...
    function _lock(
        ...
        // C2: avoid reading state multiple times
        uint256 shares = input.mulDivDown(1e18, _poolIndex(pool.balance, pool.supply));
        uint256 rewardDebt = shares.mulDivDown(pool.rewardsPerShare, 1e18);
        ...
        uint256 lhs = 1e18 - coopRateMax - uint256(1e18 - coopRateMax).mulDivDown(1e18, minPrice);
        uint256 rhs = lgm.mulDivDown(1e18, circulatingSupply() - IAccrualBondsV1(BONDS).cnvEmitted());
        require(lhs > rhs, "CAP");
    ...
    function _rebase(
        ...
        eCOOP = uint256(value - amountOut).mulDivDown(_calculateCOOPRate(value, amountOut), 1e18);
        uint256 lgm;
        uint256 erm;
        ...
        for (uint256 i; i < poolsLength;) {
            ...
            if (balance != 0) {
                lgm += balance.mulDivDown(lp.g, 1e18);
                erm += balance.mulDivDown(lp.excessRatio, 1e18);
            }
            unchecked { ++i; }
        }

        uint256 emissions = uint256(amountOut + eCOOP + eRI).mulDivDown(1e18, 1e18 - lgm.mulDivDown(1e18, CNVS));
        uint256 g = emissions.mulDivDown(1e18, CNVS);
        uint256 excessObligation = (value - emissions) + globalExcess;
        uint256 excessRewards = CNVS.mulDivDown(apyPerRebase, 1e18);
        if (excessRewards > excessObligation) excessRewards = excessObligation;
        uint256 excessMultiplier = erm != 0 ? excessRewards.mulDivDown(1e18, erm) : 0;
    ...
    function _distribute(
        ...
        for (uint256 i; i < poolsLength;) {
            ...
            if (balance != 0 && supply != 0) {
                uint256 baseObligation = g.mulDivDown(pool.g.mulDivDown(balance, 1e18), 1e18);
                uint256 excessObligation = excessMultiplier.mulDivDown(balance, 1e18).mulDivDown(pool.excessRatio, 1e18);
                emit PoolRewarded(
                    i,
                    baseObligation,
                    excessObligation,
                ...
                pool.rewardsPerShare += excessObligation.mulDivDown(1e18, supply);
    ...
    function _calculateCOOPRate(
        ...
        uint256 _bondPrice = _value.mulDivDown(1e18, _cnvOut);
    ...
    function manualExcessDistribution(
        ...
        for (uint256 i; i < length;) {
            ...
            pool.rewardsPerShare += amount.mulDivDown(1e18, pool.supply);
            toDistribute += amount;
            unchecked { ++i; }
        }
